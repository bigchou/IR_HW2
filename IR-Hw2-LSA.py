...
sum = np.sum(S)
rank_n = count = 0.0
print 'S_dim: '+str(len(S))
for i,v in enumerate(sorted(S)):
    count += v
    if count/sum >= energy:
        rank_n = i+1
        break
print 'rank_n: '+str(rank_n)
U = np.delete(U,np.s_[rank_n:len(documents)],axis=1)
S = np.delete(S,np.s_[rank_n:len(documents)],axis=0)
V = np.delete(V,np.s_[rank_n:len(documents)],axis=0) # New document vector
print V.shape

# New query vector
new_q = np.matrix(q.transpose()) * U * np.diag(S**-1)
print new_q.shape
print 'FINISH CONVERTING TO LSA MODEL'
...
for i in range(len(querys)):
    ...
    for j in range(len(documents)):
        ...
        sim[j] = 1 - spatial.distance.cosine(V[j], new_q[i])
